#! /usr/bin/python3
"""
Migrate configuration from CloudV2 to CloudV2.
  How to check if something already exists in target? Using Name or ID?
  Is there not a script already? --> by PS?
Supported operations:
  Move source, 
    Does it exists? Create else update 
    including schedules, 
    field mappings, 
    if there is an IPE configured, move that too, 
    if there is a security provider associated, move that too
  Move IPE
  Move Conditions
  Move Query Pipeline
    including all statements --> how to check if one exists already? Use Id?
  Move Custom Dimensions
  Move Analytic Reports
"""
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Migrate fields configuration from CloudV2 to CloudV2')
    ...
    v1_client = CloudV2(env, v1_org_id, v1_access_token)
    v2_client = CloudV2(env, v2_org_id, v2_access_token)

    v1_sources = v1_client.sources_get()
    if "message" in v1_sources:
      print ("Bad stuff is happening in V1 (old): "+json.dumps(v1_sources))
      exit
    v2_sources = v2_client.sources_get()
    if "message" in v2_sources:
      print ("Bad stuff is happening in V2 (new): "+json.dumps(v2_sources))
      exit



    #remove sources from v2
    print("Removing V2 (new org) sources.")
    if yes_or_no("Delete V2 (new org) sources first?"):
    ...
    finalreport+="Organization: "+v1_org_id+" (V1, old org) --> "+v2_org_id+" (V2, new org)\n"
      ...
      if not disabled:
        ...
        if addSource:
          ...
          if "ERROR" in v2_source_id:
            ...
            v1_schedules = v1_client.schedule_get(source["id"])
